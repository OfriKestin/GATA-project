...
def calc_all_p_values(path_WP_list, path_GP_list, path_WP_table, path_GP_table, merge_all, path_sig_wp, path_sig_gp):
    ...
    with open('results/delta_interactions.csv', 'w', newline='') as csv_file:
    ...
    with open('results/p_val.csv', 'w', newline='') as csv_file:
        csv_writer = csv.writer(csv_file)
...
def create_diverging_dotplot_1(merge_all, title):
    df = pd.read_csv(merge_all)
    ...
    plt.xlim(-1.05, 1.05)
    plt.savefig('results/' + title + '_diverging_dotplot.pdf')
    # plt.show()


def create_diverging_dotplot(merge_all, title):
    df = pd.read_csv(merge_all)
    df['delta'] = df.apply(lambda row: row['delta'] * -1 if row['flag'] == 1 else row['delta'], axis=1)
    df.sort_values('delta', inplace=True)
...
def create_RidgePlot(merge_all, wanted_interactions):
    # input type -> merge_all output file, list of the significant interactions as named in the interaction col
    data = pd.read_csv(merge_all)
...
if __name__ == '__main__':
    ...
    for interaction in interactions:
        create_diverging_dotplot_1('merge_all.csv', interaction)
